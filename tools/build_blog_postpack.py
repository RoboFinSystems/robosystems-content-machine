...
def hashtags(tags):
  ...
  for t in tags or []:
    out.append(
      "#" + "".join(w.capitalize() for w in str(t).replace("_", "-").split("-"))
    )
  return " ".join(out)
...
def build(slug):
  ...
  x_post = (
    companion("x_post.txt") or f"{title}\n\n{excerpt}\n\n{hashtags(tags)}".strip()
  )
  ...
  cover_url = (
    asset_url(f"blog/{slug}/cover.png")
    if os.path.exists(os.path.join(post_dir, "cover.png"))
    else None
  )
  ...
  for s in ("medium.md", "linkedin.md", "x_post.txt"):
    print(
      f"  {s:13} {'authored' if os.path.exists(os.path.join(post_dir, f'{slug}_{s}')) else 'auto-starter'}"
    )


def main():
  ap = argparse.ArgumentParser(
    description="Build a paste-ready distribution pack for a blog post"
  )
