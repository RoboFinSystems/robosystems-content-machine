...
def publish(slug, narrate=True):
  ...
  if narrate and not os.path.exists(os.path.join(post_dir, f"{slug}_narration.mp3")):
    ...
    try:
      ...
      print(
        f"  (narration skipped: {e} — publishing text only; re-run later with `just blog-narrate {slug}`)"
      )
    print()
  ...
  subprocess.run(
    ...
    input=json.dumps(meta_obj, indent=2, ensure_ascii=False),
    text=True,
    check=True,
  )
