...
def load_durations(proj: Path, ticker: str, segments, estimate: bool) -> dict:
  ...
  for seg in segments:
    mp3 = (
      proj / "videos" / "audio" / f"{ticker}_short_segment_{seg['id']}_voiceover.mp3"
    )
    if mp3.exists():
      ...
      print(
        f"ERROR: missing short voiceover {mp3.name} "
        f"(run: just webdeck-short-vo {ticker}, or pass --estimate)",
        file=sys.stderr,
      )
      sys.exit(1)
  return durations
...
def caption_chunks(narration: str, audio_start: float, audio_dur: float):
  ...
  for w in words:
    ...
    if (
      (ends_sentence and cur_chars >= 12)
      or (ends_clause and cur_chars >= 26)
      or cur_chars >= 44
      or len(cur) >= 9
    ):
...
def main() -> int:
    ...
    sections.append(
      {
        ...
        "captions": caption_chunks(
          seg["narration"], round(audio_cursor, 3), round(dur, 3)
        ),
      }
    )
    ...
    "segments": [
      {
        ...
        "file": str(
          proj / "videos" / "audio" / f"{ticker}_short_segment_{s['id']}_voiceover.mp3"
        ),
      }
      for s in sections
    ],
  ...
  print(
    f"total duration: {total}s ({mmss(total)}) across {len(sections)} beats"
