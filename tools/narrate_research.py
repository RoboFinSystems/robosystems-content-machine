...
def read_brief(ticker):
  ...
  if not bucket:
    sys.exit(
      f"Error: {local} not found and AWS_S3_BUCKET is unset — cannot fall back to S3."
    )
...
def main():
  ap = argparse.ArgumentParser(
    description="Narrate a research brief via ElevenLabs TTS"
  )
  ap.add_argument("ticker", help="Project name / ticker (e.g., STX)")
  ap.add_argument(
    "--force", action="store_true", help="Regenerate even if narration exists"
  )
  ap.add_argument(
    "--dry-run",
    action="store_true",
