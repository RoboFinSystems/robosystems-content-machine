...
ARTIFACTS = [
  ...
  (
    "reports/{t}_narration.mp3",
    "audio/mpeg",
  ),  # the audio article — a read of the brief
...
def snapshot_prior_version(bucket, ticker):
  ...
  r = subprocess.run(
    ["aws", "s3", "ls", f"s3://{bucket}/content/{ticker}/"],
    capture_output=True,
    text=True,
  )
  if r.returncode != 0:
...
def publish(project, narrate=True):
  ...
  if narrate and not os.path.exists(
    os.path.join(project_dir, "reports", f"{ticker}_narration.mp3")
  ):
    ...
    try:
      ...
      print(
        f"  (narration skipped: {e} — publishing without audio; "
  ...
  try:
    ...
    meta = {
      "ticker": ticker,
      **reindex.project_meta(ticker),
      "date": today,
      "version": reindex.quarter(today),
    }
    subprocess.run(
      [
        "aws",
      ...
      input=json.dumps(meta, indent=2, ensure_ascii=False),
      text=True,
    )
    print()
    reindex.run()
...
def main():
  ap = argparse.ArgumentParser(
    description="Publish final deliverables to the public S3 store"
  )
