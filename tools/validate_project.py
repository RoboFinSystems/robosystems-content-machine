...
def check_companion_formats(project_dir, ticker, script):
    ...
    # --- Teaser Short: auto-generated by `just short` (gen_avatar_short.py) from the brief.
    # No `short` block is authored; the tool writes videos/{ticker}_short.mp4. Nothing to validate
    # in the script - the posting copy (short_title / short_pinned_comment) is checked with the
    # publish-metadata fields below.
    ok("short: auto-generated by `just short` (avatar + word-synced captions, from the brief)")
...
def check_publish_metadata(project_dir, ticker, script):
    ...
    has_short = True  # shorts are canonical now (auto-generated avatar short)
    expected = [
        "youtube_title",
        "x_first_comment",
