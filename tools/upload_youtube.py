...
def get_creds(interactive: bool):
  ...
  if rtok:
    ...
    try:
      ...
      if not interactive:
        sys.exit(f"stored YT_REFRESH_TOKEN no longer valid ({e}) - run `just yt-auth`")
  ...
  try:
    creds = flow.run_local_server(
      port=OAUTH_PORT, access_type="offline", prompt="consent"
    )
  except Exception as e:
    sys.exit(
...
def post_first_comment(
  yt, vid: str, text: str, sidecar: Path, force: bool = False
) -> None:
  ...
  if data and data.get("comment_id") and not force:
    print(
      f"first comment already posted ({data['comment_id']}) - --force posts another"
    )
    return
  import time
  ...
  for attempt in (1, 2):
    try:
      ...
      if reason == "insufficientPermissions" or "scope" in str(e).lower():
        print(
          "WARNING: first comment not posted - token lacks the comment scope. "
      ...
      if attempt == 1:
        ...
        print(
          f"comment insert {e.status_code} ({reason or 'no reason given'}) - "
          "retrying in 20s"
        )
        time.sleep(20)
        continue
      print(
        f"WARNING: first comment not posted ({e.status_code}: {reason or e}) - "
...
def longform_url(proj: Path, ticker: str):
  ...
  try:
    return json.loads((proj / "videos" / f"{ticker}_youtube.json").read_text()).get(
      "url"
    )
  except Exception:
    return None
...
def shorts_parts(proj: Path, ticker: str, args):
  ...
  if not copy.exists():
    sys.exit(
      f"short YouTube copy not found: {copy}\n"
  ...
  for token in ("[LONGFORM_URL]", "[YOUTUBE_LINK]"):
    if token in description:
      if lf:
        ...
        print(
          f"NOTE: {token} unresolved - upload the long-form first "
  ...
  if getattr(args, "short", False):
    ...
    if ts_file.exists():
      ...
      if fixed != description:
        print(
          "chapters: rewritten from the render's timestamps "
...
def prepared_thumbnail(thumb: Path) -> Path:
  ...
  print(
    f"thumbnail {thumb.stat().st_size // 1024}KB > 2MB limit -> {out.name} "
...
def cmd_upload(args) -> int:
  ...
  sc_name = (
    f"{ticker}_short_youtube.json"
    if getattr(args, "short", False)
    else f"{ticker}_youtube.json"
  )
  ...
  if thumb:
    ...
    try:
      yt.thumbnails().set(
        videoId=vid, media_body=str(prepared_thumbnail(thumb))
      ).execute()
      print("thumbnail set")
    except HttpError as e:
      print(
        f"WARNING: thumbnail not set ({e.status_code}): wrong channel or "
  ...
  if body["status"]["privacyStatus"] != "public":
    print(
      "NOTE: uploaded non-public. If Studio shows it LOCKED private, the GCP "
...
def acting_channel_guard(yt):
  ...
  if want and cid != want:
    sys.exit(
      f"ABORT: token is bound to '{title}' ({cid}) but .env pins "
...
def cmd_auth(_args) -> int:
  ...
  print(
    f'pin it: add YT_CHANNEL_ID="{cid}" to .env so uploads refuse any other '
...
def main() -> int:
  ...
  pub.add_argument(
    "--short", action="store_true", help="publish the Short (its own sidecar)"
  )
  ...
  cm.add_argument(
    "--short", action="store_true", help="comment on the Short (its own sidecar)"
  )
