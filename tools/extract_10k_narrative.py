  ...
  for item_num, label in target_items.items():
    ...
    for m in matches:
      ...
      if other_items <= 1:
        ...
        sections[item_num] = {"start": pos, "label": label, "heading": heading}
        break
...
def main():
  if len(sys.argv) < 3:
    print(
      "Usage: python3 extract_10k_narrative.py <input.htm> <output.txt> [--ticker TICKER] [--full] [--form 10-K|10-Q]"
    )
    sys.exit(1)
  ...
  if form not in FORM_SECTIONS:
    print(
      f"ERROR: unsupported --form {form} (expected one of {', '.join(FORM_SECTIONS)})"
    )
    sys.exit(1)
  ...
  if "--full" in sys.argv:
    ...
    with open(output_path, "w") as f:
      ...
      f.write(
        "# Financial tables are included here; prefer XBRL via RoboSystems MCP for figures.\n\n"
      )
      f.write(full)
  ...
  output_parts.append("# Auto-extracted from SEC filing — qualitative content only")
  output_parts.append(
    "# Financial tables and XBRL data excluded (available via RoboSystems MCP)"
  )
  output_parts.append("")
