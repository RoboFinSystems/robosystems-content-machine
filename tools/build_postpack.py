...
MEDIA = {
  ...
  "short": (
    "videos/{t}_short.mp4",
    "{t}_short.mp4",
  ),  # 9:16 (music variant) -> X post + YT Short
  "thumbnail": (
  ...
  "thumbnail_sq": (
    "charts/png/{t}_thumbnail_square.png",
    "{t}_thumbnail_square.png",
  ),  # 1:1, orphaned with the podcast
}
...
def finalize_chapters(yt_desc, chapters_text):
  ...
  start = next(
    (
      ...
      if re.search(r"chapters", ln, re.I)
      and not ts.match(ln)
      and i + 1 < len(lines)
      and ts.match(lines[i + 1])
    ),
    None,
  )
  if start is None:
...
def build(project):
  ...
  brief_url = (
    asset_url(f"content/{t}/{t}_brief.md") if os.path.exists(brief_local) else None
  )
  ...
  if urls["final"]:
    ...
    lines.append(
      block(final_desc)
      if final_desc
      else f"_(missing social/{t}_youtube_description.txt)_"
    )
  ...
  if main_x_copy:
    ...
    lines.append(
      "**Step 1 — publish the brief as an X Article FIRST** "
    ...
    if brief_text:
      ...
      lines.append(
        f"- Highlight the rendered brief below + paste as the X Article{ref}:"
      )
      lines.append("")
    ...
    lines.append("")
    lines.append(
      "**Step 2 — the main post** (native video + a link to the Article from Step 1):"
    )
    ...
    if vid:
      ...
      lines.append(
        f"- **Image** (attach the 5:2 X thumbnail so it isn't a bare-text post): {img}"
      )
    post_with_link = f"{x_body}\n\n📄 Full brief: [X_ARTICLE_LINK]"
    lines.append(
      f"- **Post** ({len(x_body)} chars + the Article link — one post, NOT a thread):"
    )
  ...
  if has_short:
    ...
    if short_yt:
      ...
      s_lines.append(
        f"_(author social/{t}_short_youtube.txt — line 1 = title, the rest = "
  ...
  fill = [
    f"- `{tok}` — {PLACEHOLDER_HELP.get(tok, 'fill before posting')}" for tok in tokens
  ]
  ...
  if has_short:
    order.append(
      "2. **YouTube Short** (the 9:16) once the long-form is live, so its description "
      "links the long-form"
    )
  order.append(
