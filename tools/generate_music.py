...
def generate(prompt, length_s, out_id, append_manifest):
  ...
  for path in ENDPOINTS:
    ...
    print(
      f"    -> status={status} content-type={ctype} bytes={len(payload) if payload else 0}"
    )
...
def main():
  p = argparse.ArgumentParser(
    description="Generate a music bed via ElevenLabs Music API"
  )
  p.add_argument(
    "prompt", help=f"A preset name ({', '.join(PRESETS)}) or a literal prompt"
  )
  p.add_argument(
    "--length", type=float, default=30, help="Length in seconds (default 30)"
  )
  p.add_argument(
    "--id", default=None, help="Output id (default: preset name or 'generated')"
  )
  p.add_argument(
    "--append-manifest", action="store_true", help="Add to assets/music/manifest.json"
  )
  args = p.parse_args()
