...
def yt_section(days_list, top_n):
  ...
  try:
    ch = (
      data.channels().list(part="statistics,snippet", mine=True).execute()["items"][0]
    )
    st = ch["statistics"]
    print(
      f"{ch['snippet']['title']}  -  subs {st.get('subscriberCount')}, "
  ...
  CORE = (
    "views,estimatedMinutesWatched,averageViewPercentage,subscribersGained,likes,shares"
  )
  for d in days_list:
    try:
      cols, rows = q(CORE, d)
      ...
      print(
        f"  {d:>4}d: {v.get('views')} views, {v.get('estimatedMinutesWatched')} watch-min, "
  ...
  try:
    ...
    for r in rows:
      print(
        f"  {r[0]:22} {r[1]:>5} views ({r[1] / tot * 100:>4.0f}%)  {r[2]} watch-min"
      )
  except Exception as e:
    print(f"  error {e}")
  ...
  try:
    cols, rows = q(
      "views,averageViewPercentage,estimatedMinutesWatched,subscribersGained",
      365,
      dimensions="video",
      sort="-views",
      maxResults=top_n,
    ...
    for i in range(0, len(vids), 50):
      for it in (
        data.videos()
        .list(part="snippet,contentDetails", id=",".join(vids[i : i + 50]))
        .execute()
        .get("items", [])
      ):
        titles[it["id"]] = (
          it["snippet"]["title"],
          iso_dur(it["contentDetails"]["duration"]),
        )
    for r in rows:
      t, dur = titles.get(r[0], ("?", "?"))
      print(
        f"  {r[1]:>4} views  {float(r[2]):>5.1f}% AVD  {dur:>5}  +{r[4]} subs   {t[:56]}"
      )
  except Exception as e:
    print(f"  error {e}")
...
def x_section(n_posts):
  ...
  try:
    s = OAuth1Session(
      ...
      client_secret=os.environ["X_SECRET_KEY"],
  ...
  while len(posts) < n_posts:
    params = {
      "max_results": min(100, n_posts - len(posts)),
  ...
  def classify(p):
    ...
    """
    urls = [
      u.get("expanded_url", "") or "" for u in p.get("entities", {}).get("urls", [])
    ]
  ...
  for r in rows:
    print(
      f"{r['date']:11}{r['imp']:>7}{r['eng']:>5}{r['er']:>5.1f}%{r['bkmk']:>5} {r['kind']:5} {r['text']}"
    )
  ...
  if rows:
    ...
    for k in sorted(
      ...
      print(
        f"    {k:8} n={len(v):>3}  median {med(v):>6}  max {max(v):>6}  eng {med(e):>4.1f}%"
      )
    print(
    ...
    print(
      "  NOTE: eng% falls as impressions rise (more out-of-network reach). A high-reach"
    )
    print(
      "  post with 0.2% is not 'worse' than a 60-impression post at 8% - different jobs."
    )
...
def main():
  ...
  ap.add_argument(
    "--posts", type=int, default=100, help="how many recent X posts to pull"
  )
