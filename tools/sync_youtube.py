...
def fetch_feed(cid):
  ...
  chan = (
    yt.channels().list(part="contentDetails", mine=True).execute().get("items") or []
  )
  if not chan:
    sys.exit(
      "YouTube API returned no channel for these credentials - run `just yt-auth`"
    )
  if cid and chan[0]["id"] != cid:
    sys.exit(
      f"$YT_CHANNEL_ID is {cid} but the authenticated channel is {chan[0]['id']}"
    )
  ...
  while True:
    r = (
      yt.playlistItems()
      .list(part="snippet", playlistId=uploads, maxResults=50, pageToken=page)
      .execute()
    )
    for it in r.get("items", []):
      sn = it["snippet"]
      out.append(
        (_norm(sn["title"]), f"https://youtu.be/{sn['resourceId']['videoId']}")
      )
...
def main():
  ap = argparse.ArgumentParser(
    description="Capture YouTube URLs into the catalog via the channel RSS feed"
  )
