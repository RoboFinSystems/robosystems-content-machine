  ...
  for attempt in range(MAX_RETRIES):
    ...
    try:
      ...
      if e.code in _RETRYABLE_HTTP and attempt < MAX_RETRIES - 1:
        wait = 2**attempt
        print(
          f"  ElevenLabs {e.code} (attempt {attempt + 1}/{MAX_RETRIES}), retrying in {wait}s..."
        )
        time.sleep(wait)
        continue
...
def generate_audio(voice_id, text, output_path):
  ...
  if gap > GAP_WARN:
    print(
      f"    WARNING: {gap:.2f}s of silence mid-clip (over the {GAP_WARN:.1f}s "
      f"threshold) — listen before shipping: {output_path}",
      flush=True,
    )
  return True
...
def generate_all(project_name, force=False, short=False):
  ...
  if short:
    script_files = [
      f for f in os.listdir(scripts_dir) if f.endswith("_short_script.json")
    ]
  else:
    script_files = [
      f
  ...
  print(
    f"Script: {ticker}{' (short)' if short else ''} | {len(visual_segments)} segments need voiceover\n"
  )
  ...
  # Save manifest
  manifest_name = (
    "short_voiceover_manifest.json" if short else "voiceover_manifest.json"
  )
  manifest_path = os.path.join(audio_dir, manifest_name)
  with open(manifest_path, "w") as f:
    json.dump(
      {"ticker": ticker, "voice_id": voice_id, "segments": results}, f, indent=2
    )
...
def main():
  parser = argparse.ArgumentParser(
    description="Generate voiceover audio via ElevenLabs"
  )
  parser.add_argument("project", help="Project name (e.g., JPM_2025_10_K)")
  parser.add_argument(
    "--force", action="store_true", help="Regenerate even if audio already exists"
  )
  parser.add_argument(
    "--short",
    action="store_true",
