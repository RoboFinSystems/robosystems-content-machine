...
def main():
  ap = argparse.ArgumentParser(
    description="Mux VO + music onto a rendered demo walkthrough"
  )
  ...
  music_rel = (
    ...
    else (spec.get("music") if "music" in spec else DEFAULT_MUSIC)
  )
  music_gain = (
    args.music_gain
  ...
  if not os.path.exists(silent):
    print(
      f"ERROR: {silent} not found - render first:\n"
      f"  node renderer/src/cli.mjs demo --spec {args.spec}",
      file=sys.stderr,
    )
    return 1
  ...
  for beat in beats:
    ...
    if not ms:
      print(
        f"ERROR: beat {beat.get('id')} has no durationMs - run tools/demo_narrate.py first",
        file=sys.stderr,
      )
      return 1
    if beat.get("audio"):
  ...
  if abs(video_len - total) > 0.75:
    print(
      f"WARNING: video is {video_len:.1f}s but the spec's beats total {total:.1f}s.\n"
      f"         Re-render before publishing - the narration will drift.",
      file=sys.stderr,
    )

  if not segs:
    print(
      "ERROR: no beats carry audio - run tools/demo_narrate.py first", file=sys.stderr
    )
    return 1
  ...
  for n, s in enumerate(segs):
    ...
    labels.append(f"[a{n}]")
  vo_mix = (
    ";".join(parts) + f";{''.join(labels)}amix=inputs={len(segs)}:normalize=0[vo]"
  )
