...
def parse_rollup_when(campaign):
  ...
  if cfg.exists():
    m = re.search(
      r"^\s*rollup_when:\s*\[([^\]]*)\]", cfg.read_text(encoding="utf-8"), re.M
    )
    if m:
...
def plan_industry(key, registry, statuses):
  ...
  buckets = {
    k: [] for k in ("rolled", "queued", "stale", "blocked", "discovered", "funds")
  }
  plan = []
  for m in uni.get("members", []):
    status = (m.get("coverage") or {}).get("status", "")
    hit = (by_cik.get(m.get("cik")) if m.get("cik") else None) or by_slug.get(
      m["ticker"]
    )
  ...
  for f in uni.get("funds", []):
    buckets["funds"].append(
      {"tag": f["ticker"], "name": f["name"], "note": f.get("role", "")}
    )
...
def render_watchlist(key, resolved, buckets):
  ...
  for name, header in SECTIONS:
    ...
    out.append(f"## {header}")
    out += [
      f"- {b['tag']} ({b['name']}){' - ' + b['note'] if b.get('note') else ''}"
...
def main():
  ap = argparse.ArgumentParser(
    description="Roll a coverage key up into its aggregate project's sources"
  )
  ap.add_argument(
    "key",
    help="an aggregate key from classifications/keys.csv (e.g. CANNABIS, CONSUMER)",
  )
  ap.add_argument(
    "--statuses",
    help="comma list overriding campaign.yml rollup_when (industry keys only)",
  )
  ...
  statuses = (
    [s.strip().upper() for s in args.statuses.split(",")] if args.statuses else None
  )
  ...
  existing = (
    {p.name for p in briefs_dir.glob("*_brief.md")} if briefs_dir.exists() else set()
  )
  ...
  if existing:
    ...
    print(
      f"  in existing, NOT in plan: {sorted(existing - planned) or '(none)'} <- kept, review"
    )
  ...
  (agg_dir / "sources" / "_watchlist.md").write_text(
    render_watchlist(args.key, resolved, buckets), encoding="utf-8"
  )
  print(
