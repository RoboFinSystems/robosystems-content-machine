...
def load_durations(proj: Path, ticker: str, segments) -> dict:
  ...
  mp3s = [
    proj / "videos" / "audio" / f"{ticker}_segment_{seg['id']}_voiceover.mp3"
    for seg in segments
  ]
  missing = [m for m in mp3s if not m.exists()]
  if missing:
    print(
      f"ERROR: missing voiceover {missing[0]} (run voiceover first)", file=sys.stderr
    )
    sys.exit(1)
...
def main() -> int:
  ...
  ap.add_argument(
    "--no-poster", action="store_true", help="skip the thumbnail poster hold at t=0"
  )
  args = ap.parse_args()
  ...
  for s in sections:
    ...
    if (
      s.get("visual_type") == "callout"
      and slide.get("subhead")
    ...
    "segments": [
      {
        ...
        "file": str(
          proj / "videos" / "audio" / f"{ticker}_segment_{s['id']}_voiceover.mp3"
        ),
      }
      for s in sections
    ],
