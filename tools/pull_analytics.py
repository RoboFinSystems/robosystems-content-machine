...
def pull_x(session, ids):
  ...
  r = session.get(
    ...
    params={"ids": ",".join(ids[:100]), "tweet.fields": "public_metrics,created_at"},
  )
...
def pull_youtube(ana, data, video_id, scope_ok=[True]):
  ...
  for src, dst in (
    ("viewCount", "views_live"),
    ("likeCount", "likes_live"),
  ...
  try:
    ...
    if status == 403:
      scope_ok[0] = False
      row["_scope_error"] = (
        "YouTube Analytics 403 - run `just yt-auth` to re-consent "
...
def main():
  ...
  ap = argparse.ArgumentParser(
    description="Pull X + YouTube reach/retention into each project"
  )
  ap.add_argument(
    "tickers", nargs="*", help="tickers (default: every project with a stamped id)"
  )
  ...
  if not tickers:
    print(
      "No projects with a stamped X/YouTube id yet. Post something first, then "
  ...
  needs_yt = any(
    ids["youtube"] or ids["youtube_short"] for ids in ids_by_ticker.values()
  )
  ...
  for t in tickers:
    ...
    snap = {
      ...
      "youtube_short": pull_youtube(ana, data, ids["youtube_short"])
      if ids["youtube_short"]
      else None,
    }
    snapshots[t] = snap
  ...
  hdr = (
    f"{'':7}{'X impr':>9}{'X eng%':>8}{'X bkmk':>7}{'  ':2}"
  ...
  def row(label, xp, yt):
    ...
    return (
      f"{label:7}{fmt(xp.get('impressions')):>9}{er:>8}{fmt(xp.get('bookmarks')):>7}{'  ':2}"
  ...
  print("-" * 75)
  print(
    f"Snapshots appended to projects/*/analytics.json  ({len(tickers)} project(s))  ·  T·s = 9:16 short"
  )
  if scope_warned:
    print(
